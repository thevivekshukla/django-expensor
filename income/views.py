...
class SavingsCalculatorView(LoginRequiredMixin, View):
    ...
    def gen_bank_amount(self):
        ...
        for day in days:
            month_income = incomes.filter(timestamp__year=day.year, timestamp__month=day.month)
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if income:
                income = int(income)
                defaults_message.append(f'Income: <span id="curr_income" class="btn btn-default btn-xs">{income:,}</span>')
